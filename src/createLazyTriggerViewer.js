...
export function createLazyTriggerViewer(url, { type, pkgJson = {} }) {
  ...
  button.addEventListener('click', async ev => {
    if (!ev.target.hasAttribute('for-iframe-viewer-id')) {
      ...
      const responseIssueMeta = await getFromBackground({
        action: 'fetch',
        fetchProcess: 'json',
        url,
      });
      const addViewer = await createViewer(responseIssueMeta.data.body, {
        type,
